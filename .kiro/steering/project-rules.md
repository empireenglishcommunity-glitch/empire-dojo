...
## 4.6. The guide manuals keep themselves up to date — do NOT hand-edit the facts

The student manual (`site/guide/index.html`, public) and the owner manual
(`site/ops-guide/index.html`, passcode-gated) are the whole reference for
students and the owner, and they MUST stay accurate. They used to be 100%
hand-authored, so every system change silently drifted a fact (the student guide
claimed "~340 words" when A1 actually has ~315; the ops flag catalog was missing
whole initiatives and listed the retired MI'YAR). That is fixed structurally:

**The derivable facts are GENERATED, the prose is hand-written.**
- Facts (level table + week counts + vocab targets + advancement scores, the
  full feature-flag catalog with defaults, counts, timezone/schedule) are read
  live from the real system by `scripts/guide_facts.py` (sources:
  `empire-nexus` `config.CEFR_LEVELS`, `flag_registry.REGISTRY`, the curriculum
  `data/*.json`) and injected by `scripts/guide_sync.py` into fenced regions:
  ```html
  <!-- AUTO:key START · generated by scripts/guide_sync.py — do not edit by hand -->
  ...generated...
  <!-- AUTO:key END -->
  ```
- **NEVER hand-edit anything between AUTO markers** — it will be overwritten and
  the CI drift-check will fail. To change a generated fact, change it at its
  source (usually in empire-nexus) and re-run the sync.
- Editorial prose (how a feature is explained in Arabic, rights/privacy wording,
  tone) is OUTSIDE the markers and stays hand-written. A script must never
  rewrite it — that is how you'd introduce the "misleading information" this
  whole system exists to prevent.

**It runs automatically.** `scripts/generate.py` calls `guide_sync` at the end of
every build, so regenerating the site refreshes the guides in the same step. The
per-page `guide-build:` / `ops-guide-build:` date marker auto-bumps when a region
actually changes.

**Two-layer safety net in CI** (`scripts/verify_guides.py`, wired into
`dojo-verify.yml`):
1. HARD FAIL if any AUTO region is stale (someone hand-edited generated HTML, or
   the system changed without a re-sync). Fix with `python3 scripts/guide_sync.py`.
2. WARN if hand-written editorial prose disagrees with a live fact (e.g. a
   schedule time, the level count) — surfaced for a human to fix, not
   auto-changed. Some facts (itqan pass %) are runtime-tunable in the DB, so the
   code default is advisory only — hence warn, never fail.

**THE RULE for any agent/owner changing the EEC system:** after a change that
touches curriculum structure, levels, feature flags, or schedule, just re-run
`EEC_REPO_DIR=<path> python3 scripts/generate.py` (or `scripts/guide_sync.py`
alone) and commit — the guides update themselves. If you add a NEW fact category
