...
const DarbExercise = {
  ...
  init() {
    ...
    const match = window.location.pathname.match(/\/(l\d|[abc][12])\/week(\d+)\/day(\d+)\/(accent|shadowing|listening|vocab|speaking|grammar|reading)/);
    if (!match) return;
...
const DarbDayProgress = {
  async apply() {
    ...
    const exMatch = window.location.pathname.match(/\/(accent|shadowing|listening|vocab|speaking|grammar|reading)/);
    if (exMatch) {
